...
$expectedPathSetSha256 = '20d7acae305a559c66a7562d29b5cc13f009dd28db510d2ccf0bc8b5cdbe58e3'
...
$schemaVersion = 1
$evidenceRelativeRoot = 'docs/evidence/435_GATE8_COMMAND_LOCAL_GIT_TRUST_REPAIR1_SINGLE_DIAGNOSTIC_STARTUP'
$harnessRelativePath = "$evidenceRelativeRoot/diagnostic_run_harness_repair1.ps1"
...
function Invoke-RepositoryGitRead {
    ...
    $normalizedRepositoryRoot = [IO.Path]::TrimEndingDirectorySeparator(
        [IO.Path]::GetFullPath($RepositoryRoot)).Replace('\', '/')
    if ($normalizedRepositoryRoot -ne 'C:/EtcProject/FamilyClaimRef') {
...
if ($baselinePaths.Count -ne 56 -or
...
$observation = [ordered]@{
    ...
    priorHarnessInvocationCount = 1
    priorProductStartAttemptCount = 0
    repair1HarnessInvocationCount = 1
    cumulativeProductStartAttemptCount = $productStartAttemptCount
    elevatedGitTrustProbeExecuted = $true
    elevatedGitTrustProbePassed = $true
    ...
    priorHarnessSha256 = $expectedPriorHarnessSha256
    priorHoldDocumentSha256 = $expectedPriorHoldDocumentSha256
