...
$expectedPathSetSha256 = '9b15b71d81a7f76d512b359c46d91223bb9446f70673f6fb5eaa2277d7b2dbbe'
$expectedT0AggregateSha256 = '8ab50ce002109abb81f841f057a4d2aaff87d90c1d3c96c07041eeefd4590f0f'
...
$schemaVersion = 1
$evidenceRelativeRoot = 'docs/evidence/436_GATE8_WINDOWS_POWERSHELL_PATH_COMPATIBILITY_REPAIR2_SINGLE_DIAGNOSTIC_STARTUP'
$harnessRelativePath = "$evidenceRelativeRoot/diagnostic_run_harness_repair2.ps1"
...
function Get-RepositoryManifestAggregateSha256 {
    param([Parameter(Mandatory)][string]$RepositoryRoot)
    ...
    foreach ($line in $lines) {
        ...
        if ($relativePath -eq $harnessRelativePath -or
            $relativePath -eq $observationRelativePath -or
            $relativePath -eq $logEvidenceRelativePath -or
            $relativePath -like 'docs/436_*') {
            continue
        }

        $state = if ($line.StartsWith('??')) { 'untracked' } else { 'tracked' }
        $fullPath = Join-Path $RepositoryRoot ($relativePath.Replace('/', '\'))
        if (-not (Test-Path -LiteralPath $fullPath -PathType Leaf)) {
            throw 'T0_MANIFEST_PATH_MISSING'
        }

        $bytes = (Get-Item -LiteralPath $fullPath).Length
        $sha256 = Get-FileSha256 $fullPath
        $manifestLines.Add("$relativePath`t$state`t$bytes`t$sha256")
    }

    $manifestArray = $manifestLines.ToArray()
    [Array]::Sort($manifestArray, [StringComparer]::Ordinal)
    return Get-StringSha256 (($manifestArray -join "`n") + "`n")
}
...
if ($baselinePaths.Count -ne 58 -or
    (Get-StringSha256 $baselinePathSetText) -ne $expectedPathSetSha256) {
    throw 'Repository path-set baseline mismatch.'
}
$baselineT0AggregateSha256 =
    Get-RepositoryManifestAggregateSha256 $repositoryRoot
if ($baselineT0AggregateSha256 -ne $expectedT0AggregateSha256) {
    throw 'Repository T0 manifest baseline mismatch.'
}
$baselineContentMap = Get-RepositoryContentMap $repositoryRoot $baselinePaths

if (-not (Test-Path -LiteralPath $priorHoldDocumentPath -PathType Leaf) -or
    (Get-Item -LiteralPath $priorHoldDocumentPath).Length -ne
        $expectedPriorHoldDocumentBytes -or
    (Get-FileSha256 $priorHoldDocumentPath) -ne
        $expectedPriorHoldDocumentSha256 -or
    -not (Test-Path -LiteralPath $priorHarnessPath -PathType Leaf) -or
    (Get-Item -LiteralPath $priorHarnessPath).Length -ne
        $expectedPriorHarnessBytes -or
    (Get-FileSha256 $priorHarnessPath) -ne
        $expectedPriorHarnessSha256 -or
    -not (Test-Path -LiteralPath $priorHold435Path -PathType Leaf) -or
    (Get-Item -LiteralPath $priorHold435Path).Length -ne
        $expectedPriorHold435Bytes -or
    (Get-FileSha256 $priorHold435Path) -ne
        $expectedPriorHold435Sha256 -or
    -not (Test-Path -LiteralPath $priorRepair1HarnessPath -PathType Leaf) -or
    (Get-Item -LiteralPath $priorRepair1HarnessPath).Length -ne
        $expectedPriorRepair1HarnessBytes -or
    (Get-FileSha256 $priorRepair1HarnessPath) -ne
        $expectedPriorRepair1HarnessSha256) {
    throw 'Protected prior evidence identity mismatch.'
}
...
$finalPaths = @(Get-RepositoryStatusPaths $repositoryRoot)
$finalPathSetMatches =
    $finalPaths.Count -eq $baselinePaths.Count -and
    (($finalPaths -join "`n") -ceq ($baselinePaths -join "`n"))
$finalT0AggregateSha256 =
    Get-RepositoryManifestAggregateSha256 $repositoryRoot
$finalT0ManifestMatches =
    $finalT0AggregateSha256 -eq $baselineT0AggregateSha256
$gitDiffCheckOutput = @(
...
$observation = [ordered]@{
    ...
    priorTotalHarnessInvocationCount = 2
    priorCumulativeProductStartAttemptCount = 0
    repair2HarnessInvocationCount = 1
    repair2ProductStartAttemptCount = $productStartAttemptCount
